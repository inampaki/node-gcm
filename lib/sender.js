...
function extractRecipient(recipient) {
    ...
    for(var i = 0; i < validKeys.length; i++){
        key = validKeys[i];
        if(recipient[key]) {
            if((key != "topic") && (key != "notificationKey") && !Array.isArray(recipient[key])){
                err = "Value in key : " + key + " must contain Array";
                break;
            }
    ...
    if(i == validKeys.length){
        err = 'Invalid recipient key(s) ' + Object.keys(recipient) + ' (valid keys: ' + validKeys.join(', ') + ')';
    }
